...
class CoordinateBoundaries {
  private coordinates: Coordinate[] = new Array(4).fill(null).map(() => {
    return { lat: 0, long: 0 };
  });
  minLat: number = 0;
  minLong: number = 0;
  ...
  constructor([...coordinates]: Coordinate[] = []) {
    this.coordinates = coordinates
      .reduce((acc, current, _index, array) => {
        if (array.length > 2) {
          acc.push(current);
          return acc;
        }
...
const SPAIN_BOUNDARIES: Array<Coordinate[]> = [
  ...
  [{ lat: 41.8, long: -9.3 }, { lat: 43.8, long: 3.33 }],
  [{ lat: 36, long: 3.3 }, { lat: 41.8, long: -7.54 }],
  //Balear islands
  [{ lat: 38.58, long: 1.15 }, { lat: 40.09, long: 4.33 }],
  //Canary islands
  [{ lat: 27.63, long: -18.18 }, { lat: 29.41, long: -13.42 }],
  //Ceuta
  [{ lat: 35.87, long: -5.383 }, { lat: 35.918, long: -5.278 }],
  //Melilla
  [{ lat: 35.264, long: -2.97 }, { lat: 35.32, long: -2.923 }]
];

class SpanishBoundaries {
...
const _spainBoundariesInstance = new SpanishBoundaries();

export {
  CoordinateBoundaries,
  _spainBoundariesInstance as SpainBoundaries,
  Coordinate,
  SpanishBoundaries
};
